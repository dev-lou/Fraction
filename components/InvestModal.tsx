...
export function InvestModal({ property, onClose, onSuccess, registryAddress }: InvestModalProps) {
    ...
    // compute modal content to avoid large inline JSX ternary (fixes parser issues)
    const modalContent = isSuccess ? (
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
            <div className="h-16 w-16 rounded-full bg-lime-400/20 flex items-center justify-center text-3xl">🎉</div>
            <h2 className={`${spaceGrotesk.className} text-2xl font-bold text-neutral-50`}>Investment Confirmed!</h2>
            <p className="text-neutral-400 text-center">You purchased {formatNumber(quantity)} tokens of {property.title} using {token}.</p>
            <div className="w-full rounded-xl border border-neutral-800 bg-neutral-900/60 p-4 text-left space-y-2">
                <div className="flex items-center justify-between text-xs text-neutral-400"><span>Tx Hash</span><span className={`${jetBrainsMono.className} text-[11px] text-lime-300 break-all`}>{txHash}</span></div>
                <div className="flex items-center justify-between text-xs text-neutral-400"><span>Registry</span><span className={`${jetBrainsMono.className} text-[11px] text-neutral-200 break-all`}>{registryAddress}</span></div>
            </div>
            <button onClick={onClose} className="mt-4 rounded-full bg-lime-400 px-6 py-2 text-sm font-semibold text-black hover:bg-lime-300 transition">Done</button>
        </div>
    ) : (
        <div>
